<?php
/**
 * Shortcodes
 *
 * A set of shortcodes that get registered with the 
 * WordPress {@link http://codex.wordpress.org/Shortcode_API ShortCode API}.
 *
 * These can be used display information such as attributitive links 
 * for the framework, the active child theme, and more.
 *
 * @package ThematicCoreLibrary
 * @subpackage Shortcodes
 */


/**
 * Display link to WP.org.
 */
function thematic5_shortcode_wp_link() {
    return '<a class="wp-link" href="http://WordPress.org/" title="WordPress" rel="generator">WordPress</a>';
}
add_shortcode('wp-link', 'thematic5_shortcode_wp_link');		  

		  
/**
 * Display link to Thematic.
 */
function thematic5_shortcode_framework_link() {
    $themelink = '<a class="theme-link" href="http://themeshaper.com/thematic/" title="Thematic Theme Framework" rel="designer">Thematic Theme Framework</a>';
    return apply_filters('thematic5_theme_link',$themelink);
}
add_shortcode('theme-link', 'thematic5_shortcode_framework_link');	


/**
 * Display link to wp-admin of the site.
 */
function thematic5_shortcode_login_link() {
    if ( ! is_user_logged_in() )
        $link = '<a href="' . site_url('/wp-login.php') . '">' . __('Login','thematic5') . '</a>';
    else
    $link = '<a href="' . wp_logout_url() . '">' . __('Logout','thematic5') . '</a>';
    return apply_filters('loginout', $link);
}
add_shortcode('loginout-link', 'thematic5_shortcode_login_link');		  	  


/**
 * Display the site title.
 */
function thematic5_shortcode_blog_title() {
	return '<span class="blog-title">' . get_bloginfo('name', 'display') . '</span>';
}
add_shortcode('blog-title', 'thematic5_shortcode_blog_title');


/**
 * Display the site title with a link to the site.
 */
function thematic5_shortcode_blog_link() {
	return '<a href="' . site_url('/') . '" title="' . esc_attr( get_bloginfo('name', 'display') ) . '" >' . get_bloginfo('name', 'display') . "</a>";
}
add_shortcode('blog-link', 'thematic5_shortcode_blog_link');


/**
 * Display the current year.
 */
function thematic5_shortcode_year() {   
    return '<span class="the-year">' . date('Y') . '</span>';
}
add_shortcode('the-year', 'thematic5_shortcode_year');


/**
 * Display the name of the parent theme.
 */
function thematic5_shortcode_theme_name() {
    return THEMENAME;
}
add_shortcode('theme-name', 'thematic5_shortcode_theme_name');


/**
 * Display the name of the parent theme author.
 */
function thematic5_shortcode_theme_author() {
    return THEMEAUTHOR;
}
add_shortcode('theme-author', 'thematic5_shortcode_theme_author');


/**
 * Display the URI of the parent theme.
 */
function thematic5_shortcode_theme_uri() {
    return THEMEURI;
}
add_shortcode('theme-uri', 'thematic5_shortcode_theme_uri');


/**
 * Display the version no. of the parent theme.
 */
function thematic5_shortcode_theme_version() {
    return THEMATICVERSION;
}
add_shortcode('theme-version', 'thematic5_shortcode_theme_version');



/**
 * Display the name of the child theme.
 */
function thematic5_shortcode_child_name() {
    return TEMPLATENAME;
}
add_shortcode('child-name', 'thematic5_shortcode_child_name');


/**
 * Display the name of the child theme author.
 */
function thematic5_shortcode_child_author() {
    return TEMPLATEAUTHOR;
}
add_shortcode('child-author', 'thematic5_shortcode_child_author');


/**
 * Display the URI of the child theme.
 */
function thematic5_shortcode_child_uri() {
    return TEMPLATEURI;
}
add_shortcode('child-uri', 'thematic5_shortcode_child_uri');


/**
 * Display the version no. of the child theme.
 * 
 */
function thematic5_shortcode_child_version() {
    return TEMPLATEVERSION;
}
add_shortcode('child-version', 'thematic5_shortcode_child_version');
